...
1.  Добавьте пропущенные данные о структуре данных (назначении столбцов)
    и преобразуйте данные в столбцах в нужный формат
...
1.  Часто вредоносное программное обеспечение использует DNS канал в
    ...
    Низкий cv_ratio (близко к 0): Указывает на высокую регулярность
    запросов. Это очень подозрительно для пользовательского трафика, но
    нормально для некоторых легитимных систем (например, мониторинг
    состояния, проверка обновлений).

    Высокий total_requests: Увеличивает вероятность, что это не
    случайность.

    Круглое mean_interval_sec: Если средний интервал точно соответствует
    60, 300 (5 минут) или 900 (15 минут) секундам, это часто указывает
    на жестко запрограммированный таймер вредоносного ПО.

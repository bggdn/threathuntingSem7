...
2. Добавьте пропущенные данные о структуре данных (назначении столбцов) и преобразуйте данные в столбцах в нужный формат
...
4. Соотношение участников обмена внутри сети и участников обращений к внешним ресурсам.
...
5. Найдите топ-10 участников сети, проявляющих наибольшую сетевую активность.
...
6.  Найдите топ-10 доменов, к которым обращаются пользователи сети и соответственное количество обращений
...
7. Опеределите базовые статистические характеристики (функция summary() ) интервала времени между последовательными обращениями к топ-10 доменам.
...
8. Часто вредоносное программное обеспечение использует DNS канал в качестве канала управления, периодически отправляя запросы на подконтрольный злоумышленникам DNS сервер. По периодическим запросам на один и тот же
домен можно выявить скрытый DNS канал. Есть ли такие IP адреса в исследуемом датасете?

    Низкий cv_ratio (близко к 0): Указывает на высокую регулярность запросов. Это очень подозрительно для пользовательского трафика, но нормально для некоторых легитимных систем (например, мониторинг состояния, проверка обновлений).

    Высокий total_requests: Увеличивает вероятность, что это не случайность.

    Круглое mean_interval_sec: Если средний интервал точно соответствует 60, 300 (5 минут) или 900 (15 минут) секундам, это часто указывает на жестко запрограммированный таймер вредоносного ПО.
